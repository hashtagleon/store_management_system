...
function loadData() {
    const p = localStorage.getItem('stock_products');
    const t = localStorage.getItem('stock_transactions');
...
function openModal(id) {
    ...
    if (id === 'resetModal') {
        ...
        if (p) { p.value = ''; setTimeout(() => p.focus(), 200); }
...
function updateStats() {
    ...
    document.getElementById('inStock').textContent = products.filter(p => p.stock > 0).length;
...
function renderStock(list) {
    ...
    tbody.innerHTML = list.map((p, idx) => {
        ...
        const stockCls = p.stock === 0 ? 'stock-low' : p.stock <= p.minStock ? 'stock-warn' : 'stock-ok';
        return `<tr>
            <td>${idx + 1}</td>
...
function renderProductDropdowns() {
    ...
    const opts = products.map(p => `<option value="${p.id}">${p.name} (স্টক: ${p.stock} ${p.unit})</option>`).join('');
...
function updateCategoryFilter() {
    ...
    sel.innerHTML = `<option value="">সব ক্যাটাগরি</option>` + cats.map(c => `<option>${c}</option>`).join('');
...
function filterTransactions() {
    ...
    const filtered = transactions.filter(t => {
        const matchSearch = !s || t.productName.toLowerCase().includes(s) || (t.person && t.person.toLowerCase().includes(s));
...
function updateIssueStock() {
    ...
    document.getElementById('issueCurrentStock').value = p ? `${p.stock} ${p.unit}` : '';
}
...
function submitIssue(e) {
    ...
    if (qty > p.stock) return showToast(`পর্যাপ্ত স্টক নেই! বর্তমান স্টক: ${p.stock} ${p.unit}`, 'error');
...
function showTxnConfirm(title, body, type) {
    document.getElementById('txnConfirmIcon').textContent = type === 'in' ? '📥' : '📤';
...
function confirmPartialDelete() {
    ...
    if (qty > p.stock) return showToast(`পরিমাণ বর্তমান স্টক (${p.stock} ${p.unit}) এর বেশি হতে পারবে না।`, 'error');
...
function addProduct(e) {
    ...
    products.push({
        id: newId, name, category, unit, stock, minStock,
        lastIn: null, lastIssue: null, lastIssueTo: null,
        createdAt: new Date().toISOString()
    });

    if (stock > 0) {
        transactions.push({
            id: Date.now(), type: 'in', productId: newId, productName: name,
            unit, qty: stock, person: 'প্রারম্ভিক স্টক', dept: '',
            note: 'নতুন পণ্য যোগ', date: new Date().toISOString().slice(0, 16)
        });
    }
...
function exportStockCSV() {
    ...
    const headers = ['#', 'পণ্যের নাম', 'ক্যাটাগরি', 'একক', 'বর্তমান স্টক', 'ন্যূনতম স্টক', 'অবস্থা'];
    const rows = products.map((p, i) => {
        const st = p.stock === 0 ? 'শেষ' : p.stock <= p.minStock ? 'কম' : 'ভালো';
        return [i + 1, `"${p.name}"`, `"${p.category}"`, p.unit, p.stock, p.minStock, st];
    });
    const csv = [headers.join(','), ...rows.map(r => r.join(','))].join('\n');
    downloadCSV(csv, `stock_register_${new Date().toISOString().slice(0, 10)}.csv`);
...
function exportTransCSV() {
    ...
    if (!list.length) return showToast('নির্বাচিত তারিখে কোনো লেনদেন নেই।', 'error');
    const headers = ['#', 'তারিখ', 'ধরন', 'পণ্যের নাম', 'পরিমাণ', 'একক', 'প্রাপক/সরবরাহকারী', 'বিভাগ/উদ্দেশ্য', 'মন্তব্য'];
    ...
    downloadCSV(csv, `transactions${suffix}_${new Date().toISOString().slice(0, 10)}.csv`);
    closeModal('exportModal');
...
function printStock() {
    ...
    const rows = products.map((p, i) => {
        const st = p.stock === 0 ? '<span style="color:#ef4444">শেষ</span>' : p.stock <= p.minStock ? '<span style="color:#f59e0b">কম</span>' : '<span style="color:#10b981">ভালো</span>';
...
function printTransactions() {
    ...
    const date = new Date().toLocaleDateString('bn-BD', { year: 'numeric', month: 'long', day: 'numeric' });
    const rows = list.map((t, i) => {
        const type = t.type === 'in' ? '<span style="color:#10b981">📥 IN</span>' : '<span style="color:#ef4444">📤 ISSUE</span>';
...
function formatDateTime(dateStr) {
    if (!dateStr) return '—';
    const d = new Date(dateStr);
    return d.toLocaleString('bn-BD', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
}
...
function updateLowStockBanner() {
    ...
    if (lowItems.length === 0) { banner.style.display = 'none'; return; }
    const outOfStock = lowItems.filter(p => p.stock === 0);
    const lowStock = lowItems.filter(p => p.stock > 0 && p.stock <= p.minStock);
    const parts = [];
    if (outOfStock.length) parts.push(`${outOfStock.length}টি পণ্যের স্টক শেষ (${outOfStock.map(p=>p.name).slice(0,3).join(', ')}${outOfStock.length>3?'...':''})`);
    if (lowStock.length) parts.push(`${lowStock.length}টি পণ্যের স্টক কম (${lowStock.map(p=>p.name).slice(0,3).join(', ')}${lowStock.length>3?'...':''})`);
...
function saveEditProduct(e) {
    ...
    if (products.some(x => x.id !== id && x.name.toLowerCase() === newName.toLowerCase())) {
        return showToast('এই নামে অন্য পণ্য ইতিমধ্যে আছে!', 'error');
    }
    const oldName = p.name;
    p.name = newName; p.category = newCat; p.unit = newUnit; p.minStock = newMin;
    if (oldName !== newName) {
        transactions.forEach(t => { if (t.productId === id) t.productName = newName; });
    }
    saveData(); renderAll(); updateCategoryFilter();
...
function fullDeleteProduct(id) {
    ...
    document.getElementById('fullDeleteProductName').textContent = '"' + p.name + '"';
...
function confirmFullDelete() {
    ...
    transactions = transactions.filter(t => t.productId !== _fullDeleteTargetId);
...
function openLedger(id) {
    const p = products.find(x => x.id === id);
    if (!p) return;
    const pTxns = [...transactions.filter(t => t.productId === id)].sort((a,b) => new Date(b.date)-new Date(a.date));
    document.getElementById('ledgerTitle').textContent = '📦 ' + p.name + ' — ইতিহাস';
    document.getElementById('ledgerSubtitle').textContent = `ক্যাটাগরি: ${p.category} | একক: ${p.unit} | বর্তমান স্টক: ${p.stock}`;
    const totalIn = pTxns.filter(t=>t.type==='in').reduce((s,t)=>s+t.qty,0);
    const totalIssue = pTxns.filter(t=>t.type==='issue').reduce((s,t)=>s+t.qty,0);
    ...
    if (!pTxns.length) {
        ...
        tbody.innerHTML = pTxns.map((t,i) => `<tr>
...
function generateReport() {
    ...
    if (!list.length) {
        resultDiv.style.display = 'none'; emptyDiv.style.display = 'block'; return;
    }
    emptyDiv.style.display = 'none'; resultDiv.style.display = 'block';
    const inList = list.filter(t => t.type==='in').sort((a,b) => new Date(b.date)-new Date(a.date));
    const issueList = list.filter(t => t.type==='issue').sort((a,b) => new Date(b.date)-new Date(a.date));
    ...
    const rangeLabel = from && to ? `${from} থেকে ${to}` : from ? `${from} থেকে` : to ? `${to} পর্যন্ত` : 'সকল সময়';
    ...
    document.getElementById('reportInBody').innerHTML = inList.length
        ? inList.map((t,i)=>`<tr><td>${inList.length-i}</td><td>${formatDateTime(t.date)}</td><td>${t.productName}</td><td><strong>${t.qty} ${t.unit}</strong></td><td>${t.person||'—'}</td><td>${t.note||'—'}</td></tr>`).join('')
    ...
    document.getElementById('reportIssueBody').innerHTML = issueList.length
        ? issueList.map((t,i)=>`<tr><td>${issueList.length-i}</td><td>${formatDateTime(t.date)}</td><td>${t.productName}</td><td><strong>${t.qty} ${t.unit}</strong></td><td>${t.person||'—'}</td><td>${t.dept||'—'}</td><td>${t.note||'—'}</td></tr>`).join('')
...
// Patch saveData to also update the timestamp
...
function exportStockPDF() {
    ...
    try {
        ...
        doc.autoTable({
            head: [['#', 'Product Name', 'Category', 'Unit', 'Current Stock', 'Min Stock', 'Status']],
            body: rows,
            startY,
        ...
        doc.text(`Generated: ${new Date().toLocaleString()} | Total Items: ${products.length}`,
...
function exportTransPDF() {
    ...
    try {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF({ orientation: 'landscape' });
        const rangeLabel = from && to ? `${from} to ${to}` : from ? `From ${from}` : to ? `To ${to}` : 'All Transactions';
        ...
        doc.autoTable({
            head: [['#', 'Date', 'Type', 'Product', 'Qty', 'Recipient/Supplier', 'Dept/Purpose', 'Note']],
            body: rows,
            startY,
            ...
            didDrawCell: (data) => {
                if (data.column.index === 2 && data.section === 'body') {
                    data.cell.styles.textColor = data.cell.raw === 'IN' ? [16, 185, 129] : [239, 68, 68];
                    data.cell.styles.fontStyle = 'bold';
                }
            }
        });
        doc.setFontSize(7); doc.setTextColor(150);
        doc.text(`Generated: ${new Date().toLocaleString()} | Total: ${list.length} entries`,
            14, doc.internal.pageSize.getHeight() - 8);
        const suffix = from && to ? `_${from}_${to}` : '';
        doc.save(`transactions${suffix}_${new Date().toISOString().slice(0,10)}.pdf`);
        closeModal('exportModal');
...
function importBackupJSON(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            updateCategoryFilter();
            closeModal('exportModal');
            showToast(`✅ ব্যাকআপ সফলভাবে লোড হয়েছে! ${products.length}টি পণ্য, ${transactions.length}টি লেনদেন।`, 'success');
        } catch (err) {
